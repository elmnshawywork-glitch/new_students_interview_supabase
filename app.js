...
RENDER.interview = async function(){
  ...
  var html='<div class="card"><h3>تقييم المقابلة</h3>'+
    ...
    '<div class="field"><label>اختر الطالب (غير المُقيَّمين أولاً)</label><select id="iv-stu" size="1"><option value="">— اختر —</option>'+
    students.map(function(s){var done=(s.evaluatorsCount||0)>0; return '<option value="'+esc(s.nationalId)+'" data-search="'+esc((s.name||"")+" "+s.nationalId)+'">'+(done?"✔ ":"• ")+esc(s.name)+' — '+esc(s.nationalId)+(done?" (مُقيَّم)":"")+'</option>';}).join("")+'</select></div>'+
    '<div id="iv-head"></div>'+
...
async function importResultsFile(file){
  ...
  rows.forEach(function(r){
    ...
    var unit=n(pick(r,["درجةالوحده","الوحده","unitgrade"]));  // درجة الوحدة الخام
    if(unit!=null) grades.push({nationalId:nid,name:name,grade:unit});
    var lang=n(pick(r,["اللغويالاجمالي","الامتحاناللغوي","langtotal","الامتحان%"]));
    if(lang!=null){
      var en=n(pick(r,["استماعانجليزي","enlisten"]))||lang, tr=n(pick(r,["ترجمه","translate"]))||lang, ar=n(pick(r,["استماععربي","arlisten"]))||lang;
    ...
    if(iv!=null && iv>0){ var sc=[iv,iv,iv,iv,iv,iv].map(function(x){return Math.round(x/10);}); interviews.push({nationalId:nid,scores:sc}); }
  });
...
RENDER.report = async function(){
  ...
  var html='<div class="card"><h3>تقرير النتائج</h3>'+
    ...
    '<div class="grid-2">'+
      ...
      '<div class="field"><label>أدّى امتحان</label><select id="rp-taken"><option value="">الكل</option><option value="unit">الوحدة فقط له درجة</option><option value="lang">اللغوي فقط له نتيجة</option><option value="interview">المقابلة فقط تمّت</option><option value="unit_only">الوحدة فقط (بدون لغوي/مقابلة)</option><option value="lang_only">اللغوي فقط (بدون وحدة/مقابلة)</option><option value="interview_only">المقابلة فقط (بدون وحدة/لغوي)</option><option value="none">لم يؤدِّ أي شيء</option></select></div>'+
  ...
  function filtered(){
    ...
    return window._REPORT_ROWS.filter(function(r){
      ...
      if(taken==="unit" && !hasU(r)) return false;
      if(taken==="lang" && !hasL(r)) return false;
      if(taken==="interview" && !hasI(r)) return false;
      if(taken==="unit_only" && !(hasU(r)&&!hasL(r)&&!hasI(r))) return false;
      if(taken==="lang_only" && !(hasL(r)&&!hasU(r)&&!hasI(r))) return false;
      if(taken==="interview_only" && !(hasI(r)&&!hasU(r)&&!hasL(r))) return false;
...
function reportTable(rows){
  var isAdmin=state.user.role==="admin";
  var head='<th>م</th>'+(isAdmin?'<th><input type="checkbox" id="rp-all" title="تحديد الكل"></th>':'')+'<th>الاسم</th><th>الرقم القومي</th><th>المدرسة</th><th>وحدة%</th><th>لغوي%</th><th>مقابلة%</th><th>الكلية</th><th>الحالة</th><th>يوم الامتحان</th><th></th>';
...
async function studentReport(nid){
  ...
  ov.innerHTML='<div class="print-toolbar no-print"><button class="btn btn-primary btn-sm" onclick="window.print()">🖨 طباعة / حفظ PDF</button>'+
    ...
    '<div class="print-doc">'+head+info+manualForm+scores+answersHTML+'</div>';
  ov.style.display="block";
